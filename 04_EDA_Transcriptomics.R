...
mCounts = lData$transcriptomics$data
dim(mCounts)
mCounts = t(mCounts)
mCounts[1:10, 1:10]
range(mCounts)
...
densityplot(~ values | ind, groups=samDel, data=dfData, auto.key = list(columns=4), scales=list(relation='free'))
plot(lData$covariates$CGAsampling ~ lData$covariates$CGSamDel, col=c(1,2)[as.numeric(lData$covariates$fGroups)])
...
coplot(lData$covariates$CGAsampling ~ lData$covariates$CGSamDel | lData$covariates$Csite,
       col=c(1,2)[as.numeric(lData$covariates$fGroups)])
...
fit.stan.4 = sampling(stanDso, data=lStanData, iter=3000, chains=2, pars=c('betas', 'populationMean', 'sigmaPop', 'sigmaRan',
                                                                           'nu', 'mu', 'log_lik'),
...
fit.stan.3 = sampling(stanDso, data=lStanData, iter=3000, chains=2, pars=c('betas', 'populationMean', 'sigmaPop', 'sigmaRan',
                                                                           'nu', 'mu', 'log_lik'),
                      cores=2, control=list(adapt_delta=0.99, max_treedepth = 12))
...
fit.stan.2 = sampling(stanDso, data=lStanData, iter=3000, chains=2, pars=c('betas', 'populationMean', 'sigmaPop', 'sigmaRan',
                                                                           'nu', 'mu', 'log_lik'),
                      cores=2, control=list(adapt_delta=0.99, max_treedepth = 12))
...
compare(fit.stan.4, fit.stan.3, fit.stan.2)
compare(fit.stan.4, fit.stan.3, fit.stan.2, func = LOO)
plot(compare(fit.stan.4, fit.stan.3, fit.stan.2))
...
dim(mDraws.sim)
plot(density(dfData$values), main='posterior predictive density plots, model 2')
...
plot(dfData$values[dfData$ind == 'PC1'], dfData$values[dfData$ind == 'PC2'], 
     ...
     xlab='PC1', ylab='PC2', xlim=c(-4, 5), ylim=c(-3, 4), pch=15)
...
plot(dfData$values[dfData$ind == 'PC1'], dfData$values[dfData$ind == 'PC2'], 
     ...
     xlab='PC1', ylab='PC2', xlim=c(-4, 5), ylim=c(-3, 4), pch=15, cex=1.5)
...
dim(m)
m = log(m)
colnames(m) = c('Treatment', 'Region', 'Sampling', 'SamDel', 'Residual')
