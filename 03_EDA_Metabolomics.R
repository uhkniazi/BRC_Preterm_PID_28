# File: 03_EDA_Metabolomics.R
# Auth: umar.niazi@kcl.ac.uk
# Date: 1/04/2022
# Desc: exploratory analysis for the metabolomics data from stanford dataset
...
mCounts = lData$metabolomics$data
dim(mCounts)
mCounts = t(mCounts)
mCounts[1:10, 1:10]
range(mCounts)
...
### model with 3 covariates, drop the SamDel covariate as it is
### correlated with the gasampling
m = cbind(m1, m2, m3)
lStanData = list(Ntotal=nrow(dfData), Ncol=ncol(m), X=m,
...
plot(dfData$values[dfData$ind == 'PC1'], dfData$values[dfData$ind == 'PC2'], 
     ...
     xlab='PC1', ylab='PC2', xlim=c(-3.5, 5.5), ylim=c(-2, 2), pch=15, cex=1.5)
